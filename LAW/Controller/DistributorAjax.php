...
class XP_Controller_DistributorAjax extends XP_Controller_BaseAjax
{
	...
	public function getAction()
	{
		...
		if (!empty($rows)) {
			$r = XP_BModel_Distributor::get($name, $distributor, $beginDate, $endDate, $status, $rows, $page);
			if (!empty($r)) {
				$outPut = $r;
			}
		}
	...
	public function getOneAction()
	{
		//var_dump(1);
		$outPut       = "";
		$DistributorId   = System_Lib_App::getPost('distributorID', System_Lib_Request::TYPE_INT, 0);
		$errorCode    = 0;
		$errorMessage = '';
		//var_dump($a);
		if (!empty($DistributorId)) {
			$r = XP_BModel_Distributor::getOne($DistributorId);
	...
	public function UpdateOrSaveAction()
	{
		...
		(
			...
			"endDate"   => array(
				...
				"options" => array("regexp" => "/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/i")
			),
			"mobile"    => array(
				"filter"  => FILTER_VALIDATE_REGEXP,
		...
		$model             = array();
		$model["parentId"] =1;// System_Lib_App::getPost('distributorId', System_Lib_Request::TYPE_INT, 0);
		$model["id"]       = System_Lib_App::getPost('distributorId', System_Lib_Request::TYPE_STRING, '');
		...
		$r                   = XP_BModel_Distributor::save($model);
		if (!empty($r)) {
			$outPut = $r;
		}
	...
	public function CheckEmailAction()
	{
		...
		if (!empty($email)) {
			$r = XP_BModel_Distributor::checkDistributorEmailExist($email, $id);
			if (!empty($r)) {
				$outPut = true;
			} else {
				$outPut = false;
	...
	public function CheckNameAction()
	{
		...
		$id           = System_Lib_App::getRequest('mid', System_Lib_Request::TYPE_INT);
		if (!empty($name)) {
			$r = XP_BModel_Distributor::checkDistributorNameExist($name, $id);
			if (!empty($r)) {
				$outPut = true;
			} else {
				$outPut = false;
	...
	public function deleteAction()
	{
		...
		$r = XP_BModel_Distributor::delete($id);
		if (!empty($r)) {
			$outPut = $r;
		}
		echo XP_Lib_Utility::jsonResult($errorCode, $errorMessage, $outPut);
	...
	public function batchDeleteAction()
	{
		...
		$ids          = explode(",", $ids);
		if (is_array($ids)) {
			XP_BModel_Distributor::batchDelete($ids);
		} else {
			$errorCode = "20001";
		}
		echo XP_Lib_Utility::jsonResult($errorCode, $errorMessage, $outPut);
	...
	public function batchUpdateStatusAction()
	{
		...
		$ids          = explode(",", $ids);
		if (is_array($ids)) {
			if (XP_BModel_Distributor::batchUpdateStatus($ids, $status, 0)) {

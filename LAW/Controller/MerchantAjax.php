...
class XP_Controller_MerchantAjax extends XP_Controller_BaseAjax
{
	...
	public function getAction()
	{
		...
		if (!empty($rows)) {
			$r = XP_BModel_Merchant::get($name, $distributor, $beginDate, $endDate, $status, $rows, $page);
			if (!empty($r)) {
				$outPut = $r;
			}
		}
	...
	public function getOneAction()
	{
		//var_dump(1);
		$outPut       = "N";
		$merchantId   = System_Lib_App::getPost('merchantID', System_Lib_Request::TYPE_INT, 0);
		$errorCode    = 0;
		$errorMessage = '';
		//var_dump($a);
		if (!empty($merchantId)) {
			$r = XP_BModel_Merchant::getOne($merchantId);
			if (!empty($r)) {
				$outPut = $r;
			}
		}
	...
	public function UpdateOrSaveAction()
	{
		...
		$model["id"]       = System_Lib_App::getPost('merchantId', System_Lib_Request::TYPE_STRING, '');
		...
		$model["type"]       = 3;
		$r                   = XP_BModel_Merchant::save($model);
		if (!empty($r)) {
			$outPut = $r;
		}
	...
	public function CheckEmailAction()
	{
		...
		if (!empty($email)) {
			$r = XP_BModel_Merchant::checkMerchantEmailExist($email, $id);
			if (!empty($r)) {
				$outPut = true;
			} else {
				$outPut = false;
	...
	public function CheckNameAction()
	{
		...
		$id           = System_Lib_App::getRequest('mid', System_Lib_Request::TYPE_INT);
		if (!empty($name)) {
			$r = XP_BModel_Merchant::checkMerchantNameExist($name, $id);
			if (!empty($r)) {
				$outPut = true;
			} else {
				$outPut = false;
	...
	public function deleteAction()
	{
		...
		$r = XP_BModel_Merchant::delete($id);
		if (!empty($r)) {
			$outPut = $r;
		}
		echo XP_Lib_Utility::jsonResult($errorCode, $errorMessage, $outPut);
	...
	public function batchDeleteAction()
	{
		...
		$ids          = explode(",", $ids);
		if (is_array($ids)) {
			XP_BModel_Merchant::batchDelete($ids);
		} else {
			$errorCode = "20001";
		}
		echo XP_Lib_Utility::jsonResult($errorCode, $errorMessage, $outPut);
	...
	public function batchUpdateStatusAction()
	{
		...
		$ids          = explode(",", $ids);
		if (is_array($ids)) {
			if (XP_BModel_Merchant::batchUpdateStatus($ids, $status, 0)) {
